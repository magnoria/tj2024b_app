...
class _HomeState extends State<Home> {
  ...
  void initState() {
    super.initState();
    todoFindAll(); // 해당 위젯이 최초로 열렸을때 자바에게 할일 목록 조회함수 호출
  }
      ...
      body: Center(
        child: Column(
          children: [
            ...
            // 자바로부터 가져온 할일 목록을 map 함수를 이용하여 반복해서 Card 형식의 위젯 만들기
            Expanded(
              // Column 위젯 안에서 ListView 사용시 Expanded 위젯 안에서 사용한다.
              child: ListView(
                // ListView 위젯은 높이 100% 자동 세로 스크롤을 지원하는 위젯
                //children: [ ], // [] 대신에 map 반복문
                // children: 리스트명.map((반복변수명){return 위젯();   }).toList()
                children:
                    todolist.map((todo) {
                      return Card(
                        child: ListTile(
...
// 항목 예시
// child: ListView( // ListView 위젯은 높이 100% 자동 세로 스크롤을 지원하는 위젯
// children: [
// ListTile(title: Text("항목1"),),
// ListTile(title: Text("항목1"),),
// ],
